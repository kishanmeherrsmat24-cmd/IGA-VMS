function R = vms_ex2 (varargin)
%VMS_EX2  Example 2 -- 2D exponential boundary layers (John 5.2).  I_eff = 1.
%
%   -eps Lap u + b.grad u + c u = f  on (0,1)^2,  b = (2,3), c = 1,
%   u = x y^2 - y^2 E1 - x E2 + E1 E2,  E1 = e^{2(x-1)/eps}, E2 = e^{3(y-1)/eps},
%   with exponential layers along x = 1 and y = 1.
%
...
%   THIS IS THE SLOW ONE.  Each step assembles the fine-scale response of
%   every coarse basis function, so cost grows like ndof x (cost of one
%   sub-mesh back-substitution).  Start with nel0 = 6, nsteps = 3, one degree.
%
%   METHOD.  Solver and estimator are the SAME constrained fine-scale solve on
%   V' = { v in H^1_0 : (grad v, grad vbar) = 0 for all vbar }.  The solver is
%   the exact VMS coarse problem, with no stabilisation parameter, assembled
%   in blocks of columns against ONE factorisation and solved directly, so
...
o.nsteps  = 30;         % max refinement iterations       (Inf to disable)
o.maxdof  = 1500;       % degree-of-freedom budget        (Inf to disable)
...
o.maxtime = 0;          % wall-clock budget per (p,mode) run, s.  0 = off
                        % worth setting in 2D, e.g. 'maxtime', 120
...
o.markdir = true;       % DIRECTIONAL marking: score each knot interval by
                        % sqrt(sum_j eta_ij^2) and mark in x and y separately.
                        % false = mark elements and refine the union of their
                        % intervals, which refines a CROSS.  Measured at p=3,
                        % 8 adaptive steps: same error (6.9e-3) at 450 dof
                        % directional against 3108 elementwise -- 7x, and the
                        % gap widens with every step.  Worth running both.
o.markmax = 1;          % never refine more than this FRACTION of the knot
                        % intervals in one step (1 = no cap).  On a tensor
                        % mesh every marked interval costs a whole line of
                        % elements, so 0.3 keeps the dof growth gentle.

% --- problem data ----------------------------------------------------
o.eps     = 1e-4;       % diffusion  (smaller = thinner layers, harder)

% --- HOW MUCH FINE SCALE: the space V'_h the estimator/solver live in --
o.ns      = 0;          % uniform sub-cells per coarse element per direction
                        % 0 = automatic (2; 6 at p = 1)
o.nfan    = 0;          % geometrically graded cells into the DOMAIN outflow
                        % (x = 1 and y = 1), so V'_h can hold the physical
                        % layer.  0 = automatic (halve until eps/|b_d|)
o.fsq     = 0;          % Gauss points per sub-cell.  0 = automatic (p+2)
o.substab = true;       % SUPG on the SUB-mesh.  Solver and estimator both
                        % see it, so it does not bias I_eff; it lets V'_h use
                        % fewer cells in the element sublayer.  Measured
                        % (p=3, 3 uniform meshes): I_eff 1.0000 with it,
                        % 0.9998-1.0002 without, at 20% more cost.
                        % Raising ns / nfan / fsq enriches V'_h; I_eff is
                        % insensitive once the sublayer is captured.

% --- reporting -------------------------------------------------------
o.rate    = 'sqrtdof';  % convergence order measured against
                        %   'sqrtdof'  err ~ sqrt(dof)^{-r} (mesh size, 1/h)
                        %   'dof'      err ~ dof^{-r}       (cost)
                        % in 2D sqrt(dof) ~ 1/h, so 'sqrtdof' is the order a
                        % paper quotes; 'dof' is what you pay for
...
o.plotspace = true;    % the two spaces: Vbar, G'(1_K), g'(x,y_0) and u'
o.plotproj  = true;    % u, u_h = Pu, the error field u - u_h, the computed
...
D  = sv2_ex2_data (o.eps);
mk = @(bx,by,pp,nq) sv2_space (bx, by, pp, nq, D.dlay);   % how a space is built
modes = pickmodes_ (o.mode);

if o.verbose
  fprintf ('\n=== Example 2: 2D exponential boundary layers, eps = %g ===\n', D.eps);
end
R = [];  k = 0;
for p = o.p(:).'
  fs = fsauto_ (o, p, mk);               % the fine-scale space settings
  for md = modes
    k = k + 1;
    ...
    for it = 1:nit
      ...
      [uh, in1] = sv2_vms_solve (S, O, D, [], fs, 64);   % VMS solve
...
if o.verbose,   fprintf ('\n');  summary_ ('Example 2', R, o); end
if o.plot,      plotres_ ('Example 2', R, o); end
if o.plotmesh,  plotmesh2_ ('Example 2', R, o); end
if o.plotspace, plotspace2_ ('Example 2', D, mk, o, fsauto_ (o, o.plotp, mk)); end
if o.plotproj,  plotproj2_  ('Example 2', D, mk, o, fsauto_ (o, o.plotp, mk)); end
end

% ----------------------------------------------------------------------
function fs = fsauto_ (o, p, mk)
%FSAUTO_  Resolve the "how much fine scale" options for one degree.
fs.ns = o.ns;   if fs.ns == 0, fs.ns = 2;  if p == 1, fs.ns = 6; end, end
fs.nq = o.fsq;  if fs.nq == 0, fs.nq = []; end       % [] = p+2, see SV2_FSCTX
fs.nfan = o.nfan;                                     % 0 = automatic
...
function [xq, wq, eidx] = sv_quadlayer (brk, nq, dlayer, xlay)
%SV_QUADLAYER  1D Gauss rule that resolves an exponential layer at XLAY.
%   Elements that reach into the layer are split into geometric sub-cells
%   clustering at XLAY, down to a smallest cell of size DLAYER; every other
%   element gets a single NQ-point cell.  DLAYER = [] gives a plain rule.
if nargin < 3, dlayer = []; end
if nargin < 4 || isempty (xlay), xlay = brk(end); end
[xg, wg] = sv_gauss (nq);
nel = numel (brk) - 1;
xq = [];  wq = [];  eidx = [];
for e = 1:nel
  a = brk(e);  b = brk(e+1);
  ed = [a b];
  if ~isempty (dlayer) && dlayer > 0 && abs (b - xlay) < 1e-12
    d = b - a;  cuts = [];
    while d > dlayer && numel (cuts) < 45
  ...
  for m = 1:numel(ed)-1
    ...
    xq   = [xq;   0.5*(a1+b1) + jm*xg(:)];   %#ok<AGROW>
    wq   = [wq;   jm*wg(:)];                 %#ok<AGROW>
    eidx = [eidx; e*ones(nq,1)];             %#ok<AGROW>
  end
end
end

function D = sv2_ex2_data (eps_, bvec, cval)
%SV2_EX2_DATA  Example 2 (John 5.2): two exponential boundary layers.
%   -eps Lap u + b.grad u + c u = f  on (0,1)^2,  b = (2,3), c = 1, eps = 1e-4,
%   u = x y^2 - y^2 E1 - x E2 + E1 E2,  E1 = e^{2(x-1)/eps}, E2 = e^{3(y-1)/eps}.
%   u vanishes on the whole boundary; f is the exact right-hand side (the
%   1/eps terms cancel identically, which is checked in SV2_CHECK).
if nargin < 1 || isempty (eps_), eps_ = 1e-4;    end
if nargin < 2 || isempty (bvec), bvec = [2; 3];  end
if nargin < 3 || isempty (cval), cval = 1;       end
e = eps_;  b1 = bvec(1);  b2 = bvec(2);  c = cval;
E1 = @(x) exp (2*(x-1)/e);
E2 = @(y) exp (3*(y-1)/e);
D.eps = e;  D.b = [b1; b2];  D.c = c;
D.u  = @(x,y) x.*y.^2 - y.^2.*E1(x) - x.*E2(y) + E1(x).*E2(y);
D.ux = @(x,y) y.^2 - (2/e)*y.^2.*E1(x) - E2(y) + (2/e)*E1(x).*E2(y);
D.uy = @(x,y) 2*x.*y - 2*y.*E1(x) - (3/e)*x.*E2(y) + (3/e)*E1(x).*E2(y);
D.f  = @(x,y) -2*e*x + 2*e*E1(x) + 2*y.^2 - 2*E2(y) + 6*x.*y - 6*y.*E1(x) ...
              + c*(x.*y.^2 - y.^2.*E1(x) - x.*E2(y) + E1(x).*E2(y));
D.dlay = [e/2, e/3];
D.name = 'Example 2: 2D exponential boundary layers';
end

function S = sv2_space (brkx, brky, p, nq, dlay)
%SV2_SPACE  Tensor-product B-spline space S^p_{p-1} x S^p_{p-1} on a box.
%
%   S.x, S.y   1D factors (knot vector, basis at the 1D quadrature nodes)
%   S.ndof     nx*ny, index I = ix + (iy-1)*nx  (vec of the nx-by-ny array)
%   S.free     interior degrees of freedom (homogeneous Dirichlet all round)
%   DLAY = [dx dy] switches on layer-resolving quadrature at x = 1 and y = 1.

if nargin < 4 || isempty (nq),   nq   = p + 2;  end
if nargin < 5,                   dlay = [];     end
if isempty (dlay), dlay = [0 0]; end

S.p = p;  S.brkx = brkx(:).';  S.brky = brky(:).';
S.nelx = numel (brkx) - 1;  S.nely = numel (brky) - 1;
S.nel  = S.nelx * S.nely;

for d = 1:2
  if d == 1, brk = S.brkx; dl = dlay(1); else, brk = S.brky; dl = dlay(2); end
  U = sv_knots (brk, p, p-1);
  [q, w, ei] = sv_quadlayer (brk, nq, dl);
  B = sv_evalmat (U, p, q, 2);
  F.U = U;  F.brk = brk;  F.nel = numel(brk)-1;  F.ndof = numel(U)-p-1;
  F.q = q;  F.w = w;  F.eidx = ei;
  F.B0 = B{1};  F.B1 = B{2};  F.B2 = B{3};
  W = spdiags (w, 0, numel(w), numel(w));
  F.M = F.B0.'*W*F.B0;  F.K = F.B1.'*W*F.B1;  F.C = F.B0.'*W*F.B1;
  F.h = diff (brk);
  if d == 1, S.x = F; else, S.y = F; end
end

nx = S.x.ndof;  ny = S.y.ndof;
S.nx = nx;  S.ny = ny;  S.ndof = nx*ny;
[IX, IY] = ndgrid (1:nx, 1:ny);
bnd = (IX == 1) | (IX == nx) | (IY == 1) | (IY == ny);
S.free = find (~bnd(:)).';
S.dir  = find ( bnd(:)).';
% element gather over the 2D quadrature grid (column-major in x)
S.nqx = numel (S.x.q);  S.nqy = numel (S.y.q);
[EX, EY] = ndgrid (S.x.eidx, S.y.eidx);
S.qelem = EX(:) + (EY(:)-1)*S.nelx;
S.Wq = kron (S.y.w, S.x.w);                      % tensor weights, vec order
S.E  = sparse (S.qelem, 1:numel(S.Wq), 1, S.nel, numel(S.Wq));
S.hx = repmat (S.x.h(:), S.nely, 1);             % per element sizes
S.hy = kron   (S.y.h(:), ones (S.nelx,1));
[XQ, YQ] = ndgrid (S.x.q, S.y.q);
S.xq = XQ(:);  S.yq = YQ(:);
end
